...
public class App {
  ...
  private final Integer iDefault = 51;
  ...
  private App() {
    ...
    textFieldi.setText(String.valueOf(iDefault));
  ...
  private void compute() throws IOException {
    ...
    double i = Math.toRadians(Double.parseDouble(textFieldi.getText()));

    computeTwoImpulses(k, r, R, i);
    computeThreeImpulses(k, r, R, ra, i);
  }

  private void computeTwoImpulses(double k, double r, double R, double i) throws IOException {
    BufferedWriter deltaVFile = new BufferedWriter(new FileWriter("TwoImpulses.xls"));
    deltaVFile.write("\t ΔV1 \t ΔV2 \t ΔV \n");

    twoImpulsesFirst(k, r, R, i, deltaVFile);
    twoImpulsesSecond(k, r, R, i, deltaVFile);
    twoImpulsesThird(k, r, R, i, deltaVFile);
  ...
  private void computeThreeImpulses(double k, double r, double R, double ra, double i) throws IOException {
    BufferedWriter deltaVFile = new BufferedWriter(new FileWriter("ThreeImpulses.xls"));
    deltaVFile.write("\t ΔV1 \t ΔV2 \t ΔV3 \t ΔV \n");

    threeImpulsesFirst(k, r, R, ra, i, deltaVFile);
    threeImpulsesSecond(k, r, R, ra, i, deltaVFile);
  ...
  private void twoImpulsesFirst(double k, double r, double R,
      double i, BufferedWriter deltaVFile) throws IOException {
    double deltaV1 = sqrt(k / r) * (sqrt(2.0 * R / (R + r)) - 1.0);
    double deltaV2 = sqrt(k / R + 2.0 * k * r / R / (R + r) - 2.0 * sqrt(k / R) * sqrt(2.0 * k * r / R / (R + r)) * cos(i));
    double deltaVsum = deltaV1 + deltaV2;
    deltaVFile.write("First:" + deltaV1 + "\t" + deltaV2 + "\t" + deltaVsum + "\n");
  }

  private void twoImpulsesSecond(double k, double r, double R, double i, BufferedWriter deltaVFile) throws IOException {
    double deltaV1 = sqrt(k / r + 2 * k * R / r / (R + r) - 2 * sqrt(k / r) * sqrt(2 * k * R / r / (R + r)) * cos(i));
    double deltaV2 = sqrt(k / R) - sqrt(2.0 * k * r / R / (R + r));
    double deltaVsum = deltaV1 + deltaV2;
    deltaVFile.write("Second:" + deltaV1 + "\t" + deltaV2 + "\t" + deltaVsum + "\n");
  }

  private void twoImpulsesThird(double k, double r, double R, double i, BufferedWriter deltaVFile) throws IOException {
  ...
  private void threeImpulsesFirst(double k, double r, double R, double ra,
    ...
    double deltaV2 = sqrt(pow(Va1, 2) + pow(Va2, 2) - 2 * Va1 * Va2 * cos(i));
  ...
  private void threeImpulsesSecond(double k, double r, double R, double ra,
      ...
      double deltaV3 = sqrt(
          pow(Vk, 2) + pow(Vp2, 2) - 2 * Vk * Vp2 * cos(i - radian1 - radian2));
      double deltaVsum = deltaV1 + deltaV2 + deltaV3;
      deltaVFile
